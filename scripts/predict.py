...
import numpy as np
import math
from train_mnist_knn import model


# Load the model
# model = cv2.ml.KNearest_create()
# model.train("knn_classifier")
...
for cnt in contours:
    if cv2.contourArea(cnt) > 50:
        ...
        if h > 20:
            ...
            black_image[h - int(math.floor(0.5 * h)):h + int(math.ceil(0.5 * h)),
            h - int(math.floor(0.5 * w)): h + int(math.ceil(0.5 * w))] = roi
